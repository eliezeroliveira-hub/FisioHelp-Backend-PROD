...
import {
  montarEmailVerificacaoCadastroPaciente,
  montarTextoTelefoneVerificacaoCadastroPaciente,
...
export function normalizarCpfCadastro(value) {
  const digits = onlyDigits(value);
...
function assinaturaProva({ sessaoId, exp, email, telefone, cpf }) {
  return crypto
    .createHmac('sha256', getContatoSecret())
    .update(`paciente-cadastro-contato:v1:${sessaoId}:${exp}:${email}:${telefone}:${cpf}`, 'utf8')
    .digest('base64url');
}
...
function emitirProva(row) {
  ...
  const assinatura = assinaturaProva({
    ...
    cpf: String(row.CPFNormalizado || ''),
  });
  return `v1.${sessaoId}.${exp}.${assinatura}`;
}

export function validarProvaContatoCadastro(prova, { email, telefone, cpf } = {}) {
  ...
  const cpfNorm = normalizarCpfCadastro(cpf);
  const esperado = assinaturaProva({ sessaoId, exp, email: emailNorm, telefone: telefoneNorm, cpf: cpfNorm });
...
async function garantirDisponibilidade({ email, telefone, cpf }) {
  const result = await queryWithContext(null, (req) => {
    ...
    req.input('CPF', sql.NVarChar(20), cpf);
  }, `
    SELECT
      CASE WHEN EXISTS (
      ...
      CASE WHEN EXISTS (
        SELECT 1 FROM dbo.Pacientes WHERE CPF = @CPF
      ) THEN 1 ELSE 0 END AS CPFEmUso;
  `);
  ...
  if (Number(row.CPFEmUso) === 1) throw new HttpError(409, 'CPF já cadastrado.');
}
...
function normalizarDadosIniciais(dados = {}) {
  ...
  const cpf = normalizarCpfCadastro(dados.CPF ?? dados.cpf);

  if (!nome || nome.length < 2 || nome.length > 300) throw new HttpError(400, 'Nome inválido.');
...
async function solicitarEmail(dados = {}) {
  ...
  try {
    const result = await queryWithContext(null, (req) => {
      ...
      req.input('Nome', sql.NVarChar(300), inicial.nome);
      req.input('CPF', sql.NVarChar(20), inicial.cpf);
      ...
      req.input('LimiteDiario', sql.Int, Number(ENV.PACIENTE_CADASTRO_EMAIL_DAILY_MAX || 10));
      ...
      req.input('UsuarioRegistro', sql.NVarChar(200), `CadastroPaciente:${sessaoId}`);
    }, `
      SET XACT_ABORT ON;
      ...
      DECLARE @LockResource NVARCHAR(255) = N'CadastroPacienteOtp:' +
      ...
      BEGIN
        INSERT INTO dbo.PacienteCadastroSessoes
          (Id, NomeInformado, CPFNormalizado, EmailNormalizado, TelefoneNormalizado, Status, ExpiraEm)
        VALUES
          (@Id, @Nome, @CPF, @Email, @Telefone, N'EmAndamento', DATEADD(HOUR, @SessaoExpiraHoras, SYSDATETIME()));
      END
      ...
      IF NOT EXISTS (
        SELECT 1 FROM dbo.PacienteCadastroSessoes
        WHERE Id=@Id AND CPFNormalizado=@CPF AND EmailNormalizado=@Email
          AND TelefoneNormalizado=@Telefone
      ) THROW 50404, N'CADASTRO_SESSION_MISMATCH', 1;

      IF @ConfirmarOAuth = 1
      BEGIN
        MERGE dbo.PacienteCadastroVerificacoes AS t
      ...
      BEGIN
        ...
        FROM dbo.PacienteCadastroVerificacoes WITH (UPDLOCK, HOLDLOCK)
        ...
        BEGIN
          ...
          SELECT s.*, CAST(1 AS BIT) EmailConfirmado,
            CAST(CASE WHEN EXISTS (SELECT 1 FROM dbo.PacienteCadastroVerificacoes
          ...
          FROM dbo.PacienteCadastroSessoes s WHERE s.Id=@Id;
          RETURN;
        END
        ...
        IF (SELECT COUNT_BIG(1) FROM dbo.FilaContatoTransacional WITH (UPDLOCK,HOLDLOCK)
            WHERE Tipo=N'VerificacaoCadastroPaciente' AND Canal=N'Email'
        ...
        VALUES (N'VerificacaoCadastroPaciente',N'Email',@Email,@Assunto,@Html,@Texto,@PayloadJson,
          N'Pendente',0,5,SYSDATETIME(),@UsuarioRegistro);
      END

      UPDATE s SET Status=CASE WHEN EXISTS (SELECT 1 FROM dbo.PacienteCadastroVerificacoes
          WHERE CadastroSessaoId=@Id AND Canal=N'Email' AND Status=N'Confirmado')
        AND EXISTS (SELECT 1 FROM dbo.PacienteCadastroVerificacoes
          WHERE CadastroSessaoId=@Id AND Canal=N'Telefone' AND Status=N'Confirmado')
        THEN N'ContatosConfirmados' ELSE N'EmAndamento' END,
        ContatosConfirmadosEm=CASE WHEN EXISTS (SELECT 1 FROM dbo.PacienteCadastroVerificacoes
          WHERE CadastroSessaoId=@Id AND Canal=N'Email' AND Status=N'Confirmado')
        AND EXISTS (SELECT 1 FROM dbo.PacienteCadastroVerificacoes
          WHERE CadastroSessaoId=@Id AND Canal=N'Telefone' AND Status=N'Confirmado')
...
async function solicitarTelefone(dados = {}) {
  ...
  const texto = montarTextoTelefoneVerificacaoCadastroPaciente({
    codigo,
    expiraEmMinutos: CODIGO_EXPIRA_MINUTOS,
  });
  ...
  try {
    const result = await queryWithContext(null, (req) => {
      ...
      req.input('CodigoExpiraMinutos', sql.Int, CODIGO_EXPIRA_MINUTOS);
      req.input('CooldownSegundos', sql.Int, COOLDOWN_SEGUNDOS);
      req.input('LimiteDiario', sql.Int, Number(ENV.PACIENTE_CADASTRO_TELEFONE_DAILY_MAX || 6));
      ...
      FROM dbo.PacienteCadastroSessoes WITH (UPDLOCK,HOLDLOCK) WHERE Id=@Id;
      IF @StatusSessao IS NULL THROW 50401,N'CADASTRO_SESSION_NOT_FOUND',1;
      IF @StatusSessao=N'Consumido' THROW 50402,N'CADASTRO_SESSION_CONSUMED',1;
      IF @StatusSessao IN (N'Expirado',N'Cancelado') OR @ExpiraSessao<=SYSDATETIME()
        THROW 50403,N'CADASTRO_SESSION_EXPIRED',1;
      IF @TelefoneSessao<>@Telefone THROW 50404,N'CADASTRO_SESSION_MISMATCH',1;
      IF NOT EXISTS (SELECT 1 FROM dbo.PacienteCadastroVerificacoes
        WHERE CadastroSessaoId=@Id AND Canal=N'Email' AND Status=N'Confirmado')
      ...
      IF @UltimoEnvio IS NOT NULL AND DATEDIFF(SECOND,@UltimoEnvio,SYSDATETIME())<@CooldownSegundos
        THROW 50411,N'CADASTRO_CONTACT_COOLDOWN',1;
      IF (SELECT COUNT_BIG(1) FROM dbo.FilaContatoTransacional WITH (UPDLOCK,HOLDLOCK)
          WHERE Tipo=N'VerificacaoCadastroPaciente' AND Canal=N'Telefone'
      ...
      INSERT INTO dbo.FilaContatoTransacional
        (Tipo,Canal,Destino,Assunto,Html,Texto,PayloadJson,Status,Tentativas,MaxTentativas,
         ProximaTentativaEm,UsuarioRegistro)
      VALUES (N'VerificacaoCadastroPaciente',N'Telefone',@Destino,NULL,NULL,@Texto,@PayloadJson,
        N'Pendente',0,5,SYSDATETIME(),@UsuarioRegistro);
      UPDATE dbo.PacienteCadastroSessoes SET AtualizadoEm=SYSDATETIME() WHERE Id=@Id;
      COMMIT;
      SELECT s.*,CAST(1 AS BIT) EmailConfirmado,CAST(0 AS BIT) TelefoneConfirmado,
...
async function confirmarCanal(dados = {}, canal) {
  ...
  try {
    const selected = await queryWithContext(null, (req) => {
      ...
      FROM dbo.PacienteCadastroSessoes s
      LEFT JOIN dbo.PacienteCadastroVerificacoes v
    ...
    if (!correto) {
      await queryWithContext(null, (req) => req.input('VerificacaoId', sql.BigInt, row.Id), `
        UPDATE dbo.PacienteCadastroVerificacoes
    ...
    const confirmed = await queryWithContext(null, (req) => {
      ...
      DECLARE @Ambos BIT=CASE WHEN EXISTS (SELECT 1 FROM dbo.PacienteCadastroVerificacoes
      ...
      SELECT s.*,
        ...
        CAST(CASE WHEN EXISTS (SELECT 1 FROM dbo.PacienteCadastroVerificacoes
          WHERE CadastroSessaoId=s.Id AND Canal=N'Telefone' AND Status=N'Confirmado') THEN 1 ELSE 0 END AS BIT) TelefoneConfirmado
      FROM dbo.PacienteCadastroSessoes s WHERE s.Id=@SessaoId;
    `);
...
const pacienteCadastroContatoService = {
  solicitarEmail,
...
export default pacienteCadastroContatoService;
