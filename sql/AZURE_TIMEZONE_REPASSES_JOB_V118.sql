...
BEGIN
    ...
    IF @DataInicio IS NULL SET @DataInicio = DATEADD(DAY, -7, CAST(@Agora AS DATE));
    IF @DataFim IS NULL SET @DataFim = CAST(@Agora AS DATE);
    ...
    BEGIN TRY
        ...
        ;WITH Elegiveis AS (
            ...
            WHERE
                ...
                AND t.DataCriacao < DATEADD(DAY, 1, @DataFim)
        ...
        SELECT c.FisioterapeutaId, c.TransacaoId, c.ValorLiquido,
               DATEADD(DAY, 7, CAST(@Agora AS DATE)),
               N''Pendente''
        FROM #Calculo c
        WHERE NOT EXISTS (
