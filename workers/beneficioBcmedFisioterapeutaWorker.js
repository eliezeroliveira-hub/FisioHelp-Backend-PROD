...
const TITULO_PUSH_LANCAMENTO = 'Novo benefício FisioHelp + BCMED';
const TITULO_PUSH_LEMBRETE = 'Benefício FisioHelp + BCMED';
const PUSH_LANCAMENTO =
  'A FisioHelp agora tem parceria com a BCMED. Confira seu e-mail para consultar condições especiais.';
const PUSH_LEMBRETE =
  'A parceria FisioHelp + BCMED oferece condições especiais para fisioterapeutas. Confira seu e-mail para saber como consultar.';
const TITULO_EMAIL_LANCAMENTO = 'Novo benefício FisioHelp: descontos exclusivos na BCMED';
const TITULO_EMAIL_LEMBRETE = 'Consulte seu benefício FisioHelp na BCMED';
  ...
  const result = await queryWithContext(
    ...
    `
      ...
      SET
        ...
        fn.UltimoErro = N'Campanha BCMED expirada antes do envio.',
...
function montarNotificacao(pendencia, canal, config, cicloContexto) {
  ...
  return {
    ...
    dados: {
      tipo: BCMED_DADOS_TIPO,
      emailModelo: BCMED_EMAIL_MODELO,
      ...
      fisioterapeutaId,
      fisioterapeutaNome,
      origem: 'beneficioBcmedFisioterapeutaWorker',
    },
  };
}
  ...
  const result = await queryWithContext(
    ...
    (req) => {
      ...
      req.input(
        'UsuarioRegistro',
        sql.NVarChar(200),
        'Sistema:BeneficioBcmedFisioterapeutaWorker'
      );
    },
    `
      SET XACT_ABORT ON;
      ...
      BEGIN TRY
        ...
        IF @LockResult < 0
          THROW 51000, 'Não foi possível obter o lock da campanha BCMED.', 1;
...
export async function tick() {
  ...
  let contexto;
  try {
    config = carregarConfigBeneficioBcmed(process.env);
    if (!config.enabled) {
      if (!disabledLogged) {
        disabledLogged = true;
        log(
          'info',
          'Worker do benefício BCMED desativado por BCMED_BENEFICIO_WORKER_ENABLED=false.'
        );
      }
      return;
    }
    ...
    contexto = resolverCiclosBeneficioBcmed(config, new Date());
    const usuario = usuarioSistema();
    const encerrados = await encerrarItensExpirados(usuario, contexto.hoje);
    if (encerrados > 0) {
      log('info', 'Itens expirados da campanha BCMED encerrados', {
    ...
    if (!emailAtivo && !pushAtivo) {
      ...
      if (outsideWindowLoggedFor !== logKey) {
        ...
        log('info', 'Worker do benefício BCMED fora da janela de disparo.', {
          campanhaId: config.campanhaId,
          hojeBrasil: contexto.hoje,
    ...
    try {
      ...
      log('error', 'BCMED_LINK_CHECK_FAILED', {
        campanhaId: config.campanhaId,
        erro: error?.message,
      });
      throw error;
      ...
      for (const pendencia of pendencias) {
        ...
        for (const item of canais) {
          try {
            ...
            totalFalhas += 1;
            log('warn', 'Falha individual ao enfileirar benefício BCMED', {
    ...
    log('info', 'Campanha do benefício BCMED processada', {
      campanhaId: config.campanhaId,
      hojeBrasil: contexto.hoje,
    ...
    if (totalFalhas > 0) {
      throw new BcmedCampaignError(
        'BCMED_ENQUEUE_FAILED',
        `Falha ao enfileirar ${totalFalhas} item(ns) da campanha BCMED.`
      );
    }
  } catch (error) {
    const code = error?.code || 'BCMED_ENQUEUE_FAILED';
    log('error', code, {
      campanhaId: config?.campanhaId,
      hojeBrasil: contexto?.hoje,
      erro: error?.message,
